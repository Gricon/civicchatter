...
async function router() {
  ...
  if (hash.startsWith("#/u/")) { const h = hash.split("#/u/")[1]?.toLowerCase(); await showPublicProfile(h); return; }
  if (hash.startsWith("#/d/")) { const h = hash.split("#/d/")[1]?.toLowerCase(); await showDebatePage(h); return; }
  if (hash === "#/profile") { if (!session) { location.hash = "#/login"; return; } await loadMyProfile(); showOnly(els.priv); return; }

  // default
  showOnly(els.login);
}

// small helper to disable/enable a button during async actions
function withBusy(btn, fn) {
  return async (...args) => {
    ...
    try { return await fn(...args); }
      ...
      btn.disabled = false;
      btn.textContent = btn.dataset.normalLabel || btn.textContent.replace(/…$/, "");
    }
  };
}
...
async function signupCore() {
  ...
  // ✅ FIXED: emailRedirectTo must be a string or use the constant
  ...
  // If confirmations ON, session may be null until email link clicked.
...
const login = withBusy(els.btnLogin, loginCore);
const signup = withBusy(els.btnSignup, signupCore);
...
async function showDebatePage(handle) {
  showOnly(els.debate);
  if (!handle) { els.debTitle.textContent = "Debates"; els.debDesc.textContent = "No handle given."; els.debContent.innerHTML = ""; return; }
  ...
  if (!deb || error) { els.debTitle.textContent = "Debate page not found"; els.debDesc.textContent = ""; els.debContent.innerHTML = ""; return; }
...
// -------- Wire
function wire() {
  // button default labels for withBusy()
  if (els.btnLogin) { els.btnLogin.dataset.normalLabel = "Sign In"; els.btnLogin.dataset.busyLabel = "Signing in…"; }
  if (els.btnSignup) { els.btnSignup.dataset.normalLabel = "Create Account"; els.btnSignup.dataset.busyLabel = "Creating…"; }

  els.btnLogin?.addEventListener("click", login);
  els.btnSignup?.addEventListener("click", signup);
  els.goSignup?.addEventListener("click", () => location.hash = "#/signup");
  els.goLogin?.addEventListener("click", () => location.hash = "#/login");
  els.btnSaveProfile?.addEventListener("click", saveProfile);
  els.linkLogout?.addEventListener("click", (e) => { e.preventDefault(); sb.auth.signOut().then(() => location.hash = "#/login"); });

  // submit on Enter
  els.loginPassword?.addEventListener("keydown", (e) => { if (e.key === "Enter") els.btnLogin?.click(); });
  els.signupPassword?.addEventListener("keydown", (e) => { if (e.key === "Enter") els.btnSignup?.click(); });
